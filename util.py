	...
	for next_id, document in tqdm(documents.items()):
		...
		for entity in document['document']['entities']:
			...
			try:
				...
				# If entity['text'] is not found in entity2id, add it to the file
				with open('missing_entities.txt', 'a') as file:
					file.write("{}\n".format(entity['text']))
				continue  # Skip adding to global_entity_ids since the entity is missing
...
def read_padded(my_lstm, document_emb, document_mask):
    """
    This function takes an embedded array, packs it, reads, and pads it.
    In order to use Packed_Sequence, we should sort by length, and then reverse to the original order.
    :document_emb: num_document, max_document_word, hidden_size
    :document_mask: num_document, max_document_word
    :my_lstm: LSTM module
    ...
    for i, idx in enumerate(perm_idx):
        inverse_perm_idx[idx.item()] = i  # Use .item() to convert to Python scalar
    inverse_perm_idx = torch.LongTensor(inverse_perm_idx)
...
if __name__  == "__main__":
	load_documents('datasets/10k_checkpoint/documents.json')
