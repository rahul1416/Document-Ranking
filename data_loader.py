...
class DataLoader():
    ...
    #         if question_length >= len(count_query_length):
    #             count_query_length.extend([0] * (question_length + 1 - len(count_query_length)))
            
    #         count_query_length[question_length] += 1
    #     # for sample in tqdm(self.data):
        ...
        for sample in tqdm(self.data):
            ...
            if self.use_kb:
                for i, tpl in enumerate(sample['subgraph']['tuples']):
                    sbj, rel, obj = tpl
                    try:
                        sbj_id = g2l[self.entity2id[sbj['text']]]
                        obj_id = g2l[self.entity2id[obj['text']]]
                    except KeyError as e:
                        # Log the missing entity to a file
                        with open('missing_entities.txt', 'a') as file:
                            file.write("{}\n".format(str(e)))
                        continue  # Skip this tuple and continue with the next one
                    
                    if not self.use_inverse_relation:
                        entity2fact_e += [sbj_id]
                        entity2fact_f += [i]
                        fact2entity_f += [i]
                        fact2entity_e += [obj_id]
                        self.kb_fact_rels[next_id, i] = self.relation2id[rel['text']]
                    else:
                        entity2fact_e += [sbj_id, obj_id]
                        entity2fact_f += [2 * i, 2 * i + 1]
                        fact2entity_f += [2 * i, 2 * i + 1]
                        fact2entity_e += [obj_id, sbj_id]
                        self.kb_fact_rels[next_id, 2 * i] = self.relation2id[rel['text']]
                        self.kb_fact_rels[next_id, 2 * i + 1] = self.relation2id[rel['text']] + len(self.relation2id)
            ...
            for j, entity in enumerate(sample['entities']):
                try:
                    entity_id = g2l[self.entity2id[unicode(entity['text'])]]
                    self.q2e_adj_mats[next_id, entity_id, 0] = 1.0
                except KeyError as e:
                    with open('missing_entities.txt', 'a') as file:
                        file.write("{}\n".format(str(e)))
                    continue
            ...
            for j, word in enumerate(sample['question'].split()):
                if j < self.max_query_word:
                    self.query_texts[next_id, j] = self.word2id.get(word, self.word2id['__unk__'])

            # Tokenize document
            for pid, passage in enumerate(sample['passages']):
                self.rel_document_ids[next_id, pid] = passage['document_id']

            # Construct distribution for answers (commented out)
            # self.answer_dists[next_id, g2l[self.entity2id[answer[keyword]]]] = 1.0

            self.kb_adj_mats[next_id] = (
                (np.array(entity2fact_f, dtype=int), np.array(entity2fact_e, dtype=int), np.array([1.0] * len(entity2fact_f))),
